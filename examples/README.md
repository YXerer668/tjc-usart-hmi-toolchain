...
- `xfloat_combobox_demo`: xfloat 与 combobox 的 clean rebuild 样例；`hardware_verified_2026-05-17.json` 已证明 `xval.val/vvs1` 和 `cbval.val/down/txt` 的 COM36 初值读回，交互下拉和 `qty/vvs0` 语义另算。
...
- `legacy_basic_controls_demo`: 基础 text/button/image/image-button 视觉冒烟页，不作为“全部控件完成”的覆盖证据。
