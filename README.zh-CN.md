...
- 事件调度：能编译/反解一小部分事件字节码；page1 普通按钮 `up` 事件已经实机验证过 `page 1` 跳页、显式十六进制 `printh`、同页一层 `click` 级联。媒体事件 `wav0.vid=0`、`wav0.en=1`、`play 0,0,0` 已经有官方 fixture 字节对齐证据，但实机 page-load / 通用调度还没完全闭环。
