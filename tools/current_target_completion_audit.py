    ...
    unfinished = [
        ...
        },
        {
            ...
            "why_unfinished": [
                ...
                "the latest live transport probes now also show the blocker is no longer just runtime silence: the panel is visibly powered on camera, but common and high baud rates all stay silent, the official GUI download button never enters a running state across multiple interaction methods including BM_CLICK, even a zero-length public whmi-wri entry probe receives no initial ACK, and a DTR/RTS pulse attempt shows no change",
                "a known-good SD recovery package and a desktop handoff bundle are now prepared, so the next practical dependency is external recovery rather than more local byte patching",
            ],
            "blocks": [
