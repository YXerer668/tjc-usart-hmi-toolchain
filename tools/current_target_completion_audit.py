...
def build_report() -> dict[str, Any]:
    ...
    unfinished = [
        ...
        },
        {
            ...
            "why_unfinished": [
                ...
                "a new local isolation artifact now makes that boundary explicit: under the same local multi-page route, page1 text-select (type D) and page1 file-stream (type ?) are live-positive while page1 file-browser (type A) still has field binding alive but enumeration/display dead",
                ...
                "new compiled-data elimination reports now go further and show the remaining page1 file-browser gap is not locatable anywhere inside the currently recovered compiled object-tail envelope: header/global fields expose no file-browser-specific runtime bit, attr/user headers are identical, pointer closure is valid, full page-local user records and primary records are identical, page event tables are byte-identical, prefix heads collapse back to single-page after generic multi-page normalization, and there is no post-mirror service tail with room for a hidden table",
                "the next strongest runtime discriminator is now prepared as a seed-side multi-page falsification set with ranked control strength: text-select first, file-stream second, file-browser last. if text-select and file-stream both survive on runtime page 1 while file-browser still fails, that will be very strong evidence for an A-type-specific multi-page runtime limitation rather than a generic seed-side/runtime1 failure",
                "the latest live transport probes now also show the blocker is no longer just runtime silence: the panel is visibly powered on camera, but common and high baud rates all stay silent, the official GUI download button is enabled yet never enters a running state across multiple interaction methods including BM_CLICK, even a zero-length public whmi-wri entry probe receives no initial ACK, and DTR/RTS pulse, serial BREAK pulse, plus CP210x disable/enable re-enumeration all show no change",
                "a known-good SD recovery package, verified desktop zip bundle, and clickable desktop handoff launcher are now prepared, so the next practical dependency is external recovery rather than more local byte patching",
            ],
            "blocks": [
                ...
                "seed-side runtime-limitation falsification after panel recovery",
            ],
            "evidence": [
                "examples/lifecycle_runtime_smoke/page1_runtime_mapping_reverified_2026-05-20.json",
                "examples/lifecycle_runtime_smoke/page1_filebrowser_clone_vs_local_report_2026-05-21.json",
                ...
                "examples/lifecycle_runtime_smoke/page1_local_multipt_advanced_isolation_2026-05-21.json",
                ...
                "examples/lifecycle_runtime_smoke/page1_filebrowser_compiled_data_elimination_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/seed_side_multipt_probe_fidelity_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/seed_side_multipt_runtime_limit_probes_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/transport_silence_status_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/public_whmi_entry_probe_summary_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/dtr_rts_pulse_probe_summary_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/serial_break_pulse_probe_summary_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/usb_uart_reenumeration_probe_summary_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/serial_port_inventory_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/official_gui_download_button_state_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/sd_recovery_handoff_2026-05-21.json",
                "examples/lifecycle_runtime_smoke/sd_recovery_handoff_zip_verify_2026-05-21.json",
            ],
        },
        {
    ...
    return {
        ...
        "evidence_roots": {
            ...
            "sd_recovery_handoff_zip_verify": "examples/lifecycle_runtime_smoke/sd_recovery_handoff_zip_verify_2026-05-21.json",
        },
    }
