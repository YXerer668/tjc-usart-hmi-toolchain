    ...
    status_summary.write_text(
        "\n".join(
            [
                ...
                "- 唯一相关 USB-UART 仍是 `COM36`（CP210x USB to UART Bridge）",
                "- 当前本地状态: `connect/sendme/get dim` 全静默",
                "- 常见与高波特率扫描也都静默，不是简单波特率问题",
                "- `DTR/RTS` 脉冲无变化，USB-UART disable/enable 重枚举也无变化",
                "- 官方 GUI 下载按钮是 `enabled + visible`，但多种点击法都不切换到运行态",
                ...
                "4. 如果 SD 路不方便，再考虑 `02_如需手动官方下载恢复.cmd`",
                "",
                "## 说明",
                "",
                "- 当前软件侧自动化已经基本见底。",
